...
Stokes::Stokes(std::shared_ptr<SystemDefinition> sysdef,
	       ...
	       unsigned int N_mix, Scalar coef_B1_mask, Scalar coef_B2_mask, Scalar rot_diff, Scalar T_ext)
  : IntegrationMethodTwoStep(sysdef, group),
    ...
    m_N_mix(N_mix), m_coef_B1_mask(coef_B1_mask),m_coef_B2_mask(coef_B2_mask),m_rot_diff(rot_diff),m_T_ext(T_ext)  //zhoge
    {
...
void Stokes::OutputData( unsigned int timestep, BoxDim box, Scalar current_shear_rate ){
  ...
  for (unsigned int ii = 0; ii < N; ii++) {
    ...
    fprintf (file1, "%7i %15.6e %15.6e %15.6e %12.3e %12.3e %12.3e \n",
	     tag, pos4.x, pos4.y, pos4.z, ori.x, ori.y, ori.z);
...
void Stokes::integrateStepOne(unsigned int timestep)
{
  ...
  // Initialize particle orientation on host
  ...
  if (timestep == 0)
    {
      ...
      ArrayHandle<Scalar3> h_ori0(m_pdata->getAccelerations(), access_location::host, access_mode::overwrite );  //zhoge:temp
	    
      for (unsigned int i=0; i<group_size; i++){

	//randnum = static_cast <float> (rand()) / static_cast <float> (RAND_MAX); //random number in [0.,1.)
	//randx = (randnum-0.5)*2.; //[-1.,1)
	//randnum = static_cast <float> (rand()) / static_cast <float> (RAND_MAX); //random number in [0.,1.)
	//randy = (randnum-0.5)*2.; //[-1.,1)
	//randnum = static_cast <float> (rand()) / static_cast <float> (RAND_MAX); //random number in [0.,1.)
	//randz = (randnum-0.5)*2.; //[-1.,1)
  ...
  //float rot_diff = 1./18.;  //zhoge: 18 is the persistent time due to HI. (1/180 would match it)
  noise_mean = 0.0;
  ...
  Stokes_StepOne( timestep,
		  ...
		  d_sqm_B2_mask.data,
		  d_noise_ang.data,
		  m_T_ext,  // external torque to rotate bottom heavy particles
		  m_deltaT,              
		  m_error,	       
		  current_shear_rate,    
		  16,                   //cuda block size
...
void export_Stokes(pybind11::module& m)
{
  pybind11::class_<Stokes, std::shared_ptr<Stokes> > (m, "Stokes", pybind11::base<IntegrationMethodTwoStep>())
    .def(pybind11::init< std::shared_ptr<SystemDefinition>, std::shared_ptr<ParticleGroup>, std::shared_ptr<Variant>,
	 ...
	 Scalar, Scalar, Scalar, Scalar, Scalar, Scalar, Scalar, unsigned int, Scalar, Scalar, Scalar, Scalar >() )
    //In the line above, I added ndsr, kappa, k_n, beta_AF, epsq, sqm_B1, sqm_B2, N_mix, coef_B1_mask, coef_B2_mask, rot_diff,
    //T_ext (zhoge)
