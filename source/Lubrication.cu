...
__global__ void Lubrication_RSU_kernel(
  ...
  if ( tidx < group_size ) {
    ...
    // initialize stresslet for each center particle
    Scalar si[5] = { 0.0 };
    ...
    for (neigh_idx = 0; neigh_idx < n_neigh; neigh_idx++) {
      ...
      // zhoge: The following is not needed because we work on center particles,
      //        and all particles are considered (same for all other R's).
	...
	*/
	...
	// Geometric quantities (unit vector from center to neighbor)
	Scalar3 r = make_scalar3( R.x / dist, R.y / dist, R.z / dist );
	...
	// eps_ijk * r_j * W_k
	// where eps_ijk * r_j is epsr = [  0   rz -ry ]
	//                               [ -rz  0   rx ]
	// 	                         [  ry -rx  0  ]
	//
	// (zhoge: Originally all timed by -1 due to the jo_sign. Now removed.)
	Scalar3 epsrdwi = make_scalar3( (  r.z * wi.y - r.y * wi.z ),
					( -r.z * wi.x + r.x * wi.z ),
					(  r.y * wi.x - r.x * wi.y ) );
	Scalar3 epsrdwj = make_scalar3( (  r.z * wj.y - r.y * wj.z ),
	...
	// S_yz
	si[3] += YH11 * ( r.y * epsrdwi.z + epsrdwi.y * r.z ) +
		 YH12 * ( r.y * epsrdwj.z + epsrdwj.y * r.z );

	// S_yy
	si[4] += YH11 * ( r.y * epsrdwi.y + epsrdwi.y * r.y ) +
    ...
    // Write to output (zhoge: -= because we want -R_SU*U)
...
__global__ void Lubrication_RSE_kernel(
  ...
  if ( tidx < group_size ) {
    ...
    // Initialize stresslet for center particle
    Scalar Si[5] = { 0.0 };
	...
	*/
	...
	// Geometric quantities (unit distance vector)
	Scalar rx = R.x / dist;
	Scalar ry = R.y / dist;
	Scalar rz = R.z / dist;
	Scalar3 r = make_scalar3( rx, ry, rz );
    ...
    // Write to output (+= because we want -R_SU*(-Einf))
    d_Stresslet[ 5*curr_particle ]     += Si[0];
    d_Stresslet[ 5*curr_particle + 1 ] += Si[1];
    d_Stresslet[ 5*curr_particle + 2 ] += Si[2];
    d_Stresslet[ 5*curr_particle + 3 ] += Si[3];
